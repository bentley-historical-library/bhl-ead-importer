...
# This will treat each <indexentry> as one item, creating an index item with a 'value' from the <persname>, <famname>, etc. and a 'reference_text' from the <ref>. 
...
# This forms a date string using the parent archival object's date expression, or its begin date - end date, or just it's begin date, if any exist
	daodate = ''
...
# This forms a display string using the parent archival object's title and date (if both exist), or just its title or date (if only one exists)
...
# We'll use either the <dao> title attribute (if it exists) or our display_string (if the title attribute does not exist) to form the new digital object's title
